...
class ProcessPaymentView(APIView):
  def post(self, request):
    try:
      ...
      result = gateway.payment_method.create({ #Create Braintree payment method
      ...
      if result.is_success:
        ...
        return Response(
          {'error','Failed to pay create payment method' },
          status = status.HTTP_400_BAD_REQUEST
        )

      result = gateway.transaction.sale({#Create Braintree transaction
        'customer_id': str(customer_id),
        'amount': total_amount,
